...
exports.updateChallan = exports.createChallan = exports.getChallanById = exports.getAllChallans = void 0;
const db_1 = __importDefault(require("../db"));
const client_1 = require("@prisma/client");
// Helper to generate a unique Challan number (e.g. CH-2026-0001)
const generateChallanNumber = async () => {
    const count = await db_1.default.challan.count();
    const nextNumber = String(count + 1).padStart(4, '0');
    const year = new Date().getFullYear();
    return `CH-${year}-${nextNumber}`;
};
// A. Get all challans
const getAllChallans = async (req, res, next) => {
    try {
        const challans = await db_1.default.challan.findMany({
            include: {
